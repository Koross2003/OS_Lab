...
//  la：需要映射的虚拟地址
//  create：一个bool值，用于决定是否为页表分配一个页面
// 返回值：该页表项的虚拟地址
...
static inline void page_remove_pte(pde_t *pgdir, uintptr_t la, pte_t *ptep) {
    ...
    if (*ptep & PTE_V) {  //(1) 检查此页表项是否有效
            struct Page *page =
                pte2page(*ptep);  //(2) 找到与pte对应的物理页面
            page_ref_dec(page);   //(3) 减少页面引用计数
            if (page_ref(page) ==
                0) {  //(4) 当页面引用计数达到0时，释放此页面
                free_page(page);
            }
            *ptep = 0;                  //(5) 清除第二个页表项
...
// mark 实现页面映射关键的两个函数 page_remove在页表中删除一个映射 和 page_insert在页表中建立一个映射
...
void page_remove(pde_t *pgdir, uintptr_t la) {
    // 参数含义见下
    pte_t *ptep = get_pte(pgdir, la, 0); // 找到这个页表项
    if (ptep != NULL) { // 如果该页表项不为空则删除这个映射
        page_remove_pte(pgdir, la, ptep);
    }
}

// mark 参数含义
// page_insert - 建立一个物理页面与物理地址la之间的映射关系(即创建一个页表项)
// 参数:
//  pgdir: 页表基址(satp) // mark 注意这里是三级页表的物理地址,get_pte中会根据la去二级页表中找到对应的页表项
//  page: 需要映射的物理页面
//  la: 需要映射的虚拟地址
//  perm: 在相关pte中设置的此页面的权限
// 返回值: 总是0
// 注意: PT被修改，因此需要刷新TLB
int page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) {
    pte_t *ptep = get_pte(pgdir, la, 1); // 找到对应页表项,这里的1表示如果没有找到页表项则创建一个
    if (ptep == NULL) { // 如果页表项指针为空，返回NULL
        return -E_NO_MEM;
    }
    page_ref_inc(page); // 增加页面的引用计数
    if (*ptep & PTE_V) { // 如果该页表项已经存在映射关系
        struct Page *p = pte2page(*ptep); // 获取该页表项对应的物理页面
        if (p == page) { // 如果该页表项已经映射到了该物理页面
            page_ref_dec(page); // 减少页面的引用计数
        } else { // 如果该页表项映射到了其他物理页面
            page_remove_pte(pgdir, la, ptep); // 删除该页表项的映射关系
        }
    }
    *ptep = pte_create(page2ppn(page), PTE_V | perm); // 将页表项设置为新的映射关系
...
//                  - 分配一个页面大小的内存并设置地址映射
//                  - 将虚拟地址la和PDT pgdir映射到物理地址pa
struct Page *pgdir_alloc_page(pde_t *pgdir, uintptr_t la, uint32_t perm) {
    struct Page *page = alloc_page();
    if (page != NULL) {
        if (page_insert(pgdir, page, la, perm) != 0) { // 创建页表项
            free_page(page);
            return NULL;
        }
        if (swap_init_ok) {
            swap_map_swappable(check_mm_struct, la, page, 0); // 这个0实际上没用上
            page->pra_vaddr = la;
...
void *kmalloc(size_t n) { //分配至少n个连续的字节，这里实现得不精细，占用的只能是整数个页。
    ...
    base = alloc_pages(num_pages);//如果分配失败就直接panic
    assert(base != NULL); 
