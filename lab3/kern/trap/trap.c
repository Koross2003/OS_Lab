...
static int pgfault_handler(struct trapframe *tf) {
    ...
    if (check_mm_struct != NULL) {
        return do_pgfault(check_mm_struct, tf->cause, tf->badvaddr);// mark 这里badvaddr就是发生page fault的虚拟地址
    }
...
void exception_handler(struct trapframe *tf) {
    ...
    switch (tf->cause) {
        ...
        case CAUSE_MACHINE_ECALL:
            ...
            break;
        case CAUSE_FETCH_PAGE_FAULT: // 取指令时发生的page fault先不处理 //mark
            cprintf("Instruction page fault\n");
            break;
        case CAUSE_LOAD_PAGE_FAULT:
            cprintf("Load page fault\n"); // mark tf中的badvaddr(或者叫stval)是发生page fault的虚拟地址
            if ((ret = pgfault_handler(tf)) != 0) { // do_pagefault()页面置换成功返回0
                print_trapframe(tf);
                panic("handle pgfault failed. %e\n", ret);
            }
            break;
