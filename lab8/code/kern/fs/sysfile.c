...
copy_path(char **to, const char *from) {
    ...
    lock_mm(mm);//这里的lock是通过减少信号量实现的,说实话信号量我压根没怎么听过
...
sysfile_open(const char *__path, uint32_t open_flags) {
    int ret;
    char *path;
    if ((ret = copy_path(&path, __path)) != 0) {//把用户空间传过来的路径字符串复制到内核空间
        return ret;
    }
    ret = file_open(path, open_flags);//复制之后用内核态的路径字符串打开文件
    kfree(path);
    return ret;
}
...
sysfile_read(int fd, void *base, size_t len) {
    ...
    while (len != 0) {//循环读取文件
        if ((alen = IOBUF_SIZE) > len) {//每次最多读取4096字节
            alen = len;//剩余len小于buffersize就只读取剩余的len
        }
        ret = file_read(fd, buffer, alen, &alen);//将文件内容读取到buffer中
        if (alen != 0) {
            lock_mm(mm);
            {
                if (copy_to_user(mm, base, buffer, alen)) {//将buffer中的内容复制到用户内存空间
                    assert(len >= alen);
