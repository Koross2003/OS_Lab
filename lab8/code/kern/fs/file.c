...
//    panic("debug");
    ...
    if (fd == NO_FD) {// 分配一个file结构体,就是从fd这个数组里找个是FD_NONE的返回回去
        for (fd = 0; fd < FILES_STRUCT_NENTRY; fd ++, file ++) {
        ...
        if (testfd(fd)) {
            file += fd;
            if (file->status == FD_NONE) {//如果fd不是NO_FD，那么就检查fd是否合法，然后把fd位置的file结构体分配出去(分配一个特定的fd)
                goto found;
            }
            return -E_BUSY;
        }
...
file_open(char *path, uint32_t open_flags) {
    ...
    int ret;
    struct file *file;
    // 然后给这个file分配一个file结构体
    if ((ret = fd_array_alloc(NO_FD, &file)) != 0) {//在当前进程分配file descriptor
        return ret;
    }
    struct inode *node;
    
    if ((ret = vfs_open(path, open_flags, &node)) != 0) {//通过vfs_open找到path指出的文件所对应的基于inode数据结构的VFS索引节点node
...
file_read(int fd, void *base, size_t len, size_t *copied_store) {
    ...
    *copied_store = 0;
    ...
    fd_array_acquire(file);//调用filemap_acquire函数使打开这个文件的计数加1
    ...
    if (file->status == FD_OPENED) {
        file->pos += copied;//调整文件指针偏移量pos的值，使其向后移动实际读到的字节数iobuf_used(iob)
    }
    *copied_store = copied;
    fd_array_release(file);//最后调用filemap_release函数使打开这个文件的计数减1，若打开计数为0，则释放file
    return ret;
}
