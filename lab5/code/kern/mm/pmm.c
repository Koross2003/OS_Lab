...
/* copy_range - 复制进程 A 的内存（start 到 end）的内容到进程 B
 * @to:    进程 B 的页目录的地址
 * @from:  进程 A 的页目录的地址
 * @share: 表示复制还是共享的标志。我们只使用了复制的方法，因此它没有被使用。
 *
...
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end,
               bool share) {
    // 断言确保 start 和 end 是页面对齐的
    assert(start % PGSIZE == 0 && end % PGSIZE == 0);
    // 断言确保 start 和 end 处于用户空间范围内
    assert(USER_ACCESS(start, end));


    // 按页单位复制内容
    do {
        // 调用 get_pte 根据地址 start 查找进程 A 的页表项 ptep
        pte_t *ptep = get_pte(from, start, 0), *nptep;
        if (ptep == NULL) {
            // 如果 ptep 为空，将 start 向下舍入到下一个页表边界
            start = ROUNDDOWN(start + PTSIZE, PTSIZE);
            continue;
        }


        // 调用 get_pte 根据地址 start 查找进程 B 的页表项 nptep
        if (*ptep & PTE_V) {
            // 如果 ptep 有效，说明进程 A 在地址 start 有有效的页表项

            // 如果 nptep 为空，说明进程 B 在地址 start 还没有页表项，需要分配一个页表
            if ((nptep = get_pte(to, start, 1)) == NULL) {
                return -E_NO_MEM;
            }

            // 获取权限信息，只保留 PTE_USER 位
            uint32_t perm = (*ptep & PTE_USER);
            // 通过 ptep 获取进程 A 的page
            struct Page *page = pte2page(*ptep);
            assert(page != NULL);
            
            int ret = 0;
            /* LAB5:EXERCISE2 YOUR CODE
             * 复制页面的内容到 npage，并构建物理地址与线性地址 start 之间的映射
             *
             * 一些有用的宏和定义，你可以在下面的实现中使用它们。
             * 宏或函数：
             *    page2kva(struct Page *page)：返回页面管理的内存的内核虚拟地址（请参见 pmm.h）
             *    page_insert：构建一个 Page 的物理地址与线性地址 la 之间的映射
             *    memcpy：典型的内存拷贝函数
             *
             * (1) 寻找 src_kvaddr：页面的内核虚拟地址
             * (2) 寻找 dst_kvaddr：npage 的内核虚拟地址
             * (3) 从 src_kvaddr 复制到 dst_kvaddr，大小为 PGSIZE
             * (4) 构建 nage 的物理地址与线性地址 start 之间的映射
             */
            
            
            if(share){//用不上,这个challenge做不了
                uint32_t perm_t = perm & (~PTE_W);
                //uint32_t perm_t = perm | PTE_R;
